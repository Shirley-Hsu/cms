...
from cms import config, ServiceCoord, get_service_shards, get_service_address, ConfigError, SOURCE_EXT_TO_LANGUAGE_MAP
from cms.io import WebService
from cms.db import Session, Admin, Contest, User, Announcement, Question, Message, \
...
class BaseHandler(CommonRequestHandler):
    ...
    def get_current_user(self):
        """Gets the current admin logged in from the cookies

        If a valid cookie is retrieved, return a admin object with the
        adminname specified in the cookie. Otherwise, return None.
        """
        if self.get_secure_cookie("alogin") is None:
            return None

        # Parse cookie.
        try:
            cookie = pickle.loads(self.get_secure_cookie("alogin"))
            adminname = cookie[0]
            password = cookie[1]
            last_update = make_datetime(cookie[2])
        except:
            self.clear_cookie("alogin")
            return None

        # Check if the cookie is expired.
        if self.timestamp - last_update > \
                timedelta(seconds=config.admin_cookie_duration):
            self.clear_cookie("alogin")
            return None

        # Load the admin from DB.
        admin = self.sql_session.query(Admin)\
            .filter(Admin.adminname == adminname).first()

        # Check if admin exists and is allowed to login.
        if admin is None or admin.password != password:
            self.clear_cookie("alogin")
            return None

        if self.refresh_cookie:
            self.set_secure_cookie("alogin",
                                   pickle.dumps((admin.adminname,
                                                 admin.password,
                                                 make_timestamp())),
                                   expires_days=None)
...
class LoginHandler(BaseHandler):
    ...
    """
    def post(self):
        adminname = self.get_argument("adminname", "")
        password = self.get_argument("password", "")
        next_page = self.get_argument("next", "/")
        admin = self.sql_session.query(Admin)\
            .filter(Admin.adminname == adminname).first()

        filtered_admin = filter_ascii(adminname)
        filtered_pass = filter_ascii(password)
        if admin is None or admin.password != password:
            logger.info("Login error: admin=%s pass=%s remote_ip=%s.",
                        filtered_admin, filtered_pass, self.request.remote_ip)
            self.redirect("/?login_error=true")
            return

        logger.info("Admin logged in: admin=%s remote_ip=%s.",
                    filtered_admin, self.request.remote_ip)
        self.set_secure_cookie("alogin",
                               pickle.dumps((admin.adminname,
                                             admin.password,
                                             make_timestamp())),
